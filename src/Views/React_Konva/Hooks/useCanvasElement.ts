...
export const useCanvasElements = () => {
  ...
  const addShape = (newShape: ShapeData) =>
    setShapes((prev) => [...prev, newShape]);
  const addImage = (newImage: CardImage) =>
  ...
  // Update elements
  const updateText = (id: string, updates: Partial<TextState>) =>
    setText((prev) =>
  ...
  const updateImage = (id: string, updates: Partial<CardImage>) =>
    setImages((prev) =>
